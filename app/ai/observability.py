...
THREE THINGS ARE DIFFERENT FROM ACTIVITY TRACKER'S VERSION, ALL BECAUSE
...
3. Model name has a FALLBACK, not just a lookup. response.llm_output
   sometimes omits model_name for Gemini responses (a langchain_google_genai
   quirk — Groq's ChatGroq always populates it). Since the model name is
   also known from the class's own construction kwargs at call-start time
   (e.g. ChatGoogleGenerativeAI(model="gemini-3.5-flash-lite")), we capture
   that from `serialized` in the start hooks and fall back to it if the
   end-of-call response doesn't say.
"""
import logging
import time
...
def _detect_model_hint(serialized: Optional[dict]) -> Optional[str]:
    """
    Best-effort model name from the class's own construction kwargs
    (e.g. {"kwargs": {"model": "gemini-3.5-flash-lite", ...}}), used only
    as a FALLBACK when the actual response doesn't report a model name.
    """
    if not serialized:
        return None
...
class ObservabilityCallback(AsyncCallbackHandler):
    def __init__(
        ...
        *,
        ...
        self._model_hint: Dict[UUID, Optional[str]] = {}

    # ── start hooks: record timing + which provider/model is being tried ──
    async def on_chat_model_start(self, serialized, messages, *, run_id, **kwargs) -> None:
        ...
        self._model_hint[run_id] = _detect_model_hint(serialized)
        try:
    ...
    async def on_llm_start(self, serialized, prompts, *, run_id, **kwargs) -> None:
        ...
        self._model_hint.setdefault(run_id, _detect_model_hint(serialized))
        if prompts:
    ...
    async def on_llm_end(self, response: LLMResult, *, run_id, **kwargs) -> None:
        ...
        model_hint = self._model_hint.pop(run_id, None)
        ...
        # Fallback: the response didn't report a model name (a known
        # langchain_google_genai gap) — use what we captured at call-start
        # from the class's own construction kwargs instead of leaving it empty.
    ...
    async def on_llm_error(self, error: BaseException, *, run_id, **kwargs) -> None:
        ...
        try:
            await self._log(
                _reraise, prompt_text=prompt_text, latency_ms=elapsed_ms,
